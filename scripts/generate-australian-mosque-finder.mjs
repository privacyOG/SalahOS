...
const USER_AGENT =
  'SalahOS/1.5.1 (+https://github.com/privacyOG/SalahOS; factual mosque directory refresh)';
...
const FACILITY_RULES = Object.freeze([
  ...
  [
    'women-prayer-space',
    /\b(?:women|woman|female|females|sister|sisters)\b[\s\S]{0,50}\b(?:prayer|area|room|space|facilit)/iu,
  ],
...
function normalizeType(value) {
  ...
  if (/friday|jummah|jumu.?ah/iu.test(normalized) && /only/iu.test(normalized))
    return 'friday-prayer-only';
...
function parseAddress(addressHtml, fallbackState) {
  ...
  const formattedParts = [line1, locality, state.toLocaleUpperCase('en-AU'), postcode].filter(
    Boolean,
  );
  return {
    line1,
...
function parseContact(addressHtml) {
  ...
  const email =
    ...
    firstMatch(
      addressHtml,
      /fa-(?:email|envelope)[^>]*><\/i>\s*<span[^>]*>([^<]*@[^<]*)<\/span>/iu,
    );
...
function jumuahLabel(text, offset, index) {
  ...
  const session = /(?:^|\b)(1st|2nd|3rd|4th|first|second|third|fourth)\s+session\b/iu.exec(
    before,
  )?.[1];
...
function parseDailyPrayerTimes(text) {
  const result = {};
  const prayerPattern =
    /\b(fajr|dhuhr|dhuhur|zuhr|asr|maghrib|isha|esha)\b\s*[:\-]?\s*([\s\S]*?)(?=\b(?:fajr|dhuhr|dhuhur|zuhr|asr|maghrib|isha|esha)\b|[.;]|$)/giu;
...
function parseFeatureList(html) {
  ...
  const listHtml =
    /<ul[^>]*class=["'][^"']*bullets[^"']*["'][^>]*>([\s\S]*?)<\/ul>/iu.exec(featureSection)?.[1] ??
    '';
...
function normalizedFacts(features, detailsText) {
  ...
  return {
    facilities: FACILITY_RULES.filter(([, pattern]) => pattern.test(searchable)).map(
      ([facility]) => facility,
    ),
    services: SERVICE_RULES.filter(([, pattern]) => pattern.test(searchable)).map(
      ([service]) => service,
    ),
  };
}
...
export function parseMosqueFinderPage(rawHtml, sourceUrl, retrievedAt) {
  ...
  const typeText =
    firstMatch(
      html,
      /<div[^>]*class=["'][^"']*type[^"']*["'][^>]*>[\s\S]*?<span[^>]*>([\s\S]*?)<\/span>/iu,
    ) ?? 'other';
...
function validateRecord(record) {
  assert(
    typeof record.id === 'string' && record.id.startsWith('mosque-finder:'),
    'Mosque Finder ID is invalid',
  );
  assert(
    typeof record.name === 'string' && record.name.length > 0,
    `Missing name for ${record.id}`,
  );
  assert(
    Object.values(REGION_CODES).includes(record.address?.regionCode),
    `Invalid region for ${record.id}`,
  );
  assert(
  ...
  assert(
    record.latitude >= -44.5 && record.latitude <= -9,
    `Latitude outside Australia for ${record.id}`,
  );
  assert(
    record.longitude >= 112 && record.longitude <= 154.5,
    `Longitude outside Australia for ${record.id}`,
  );
  assert(
    typeof record.sourceUrl === 'string' && record.sourceUrl.startsWith(`${BASE_URL}/mosque/`),
    `Invalid source URL for ${record.id}`,
  );
...
export function validateDirectory(directory) {
  ...
  assert(
    directory.source?.name === 'Australian Mosque Finder',
    'Mosque Finder source label is invalid',
  );
  assert(
    Array.isArray(directory.records) && directory.records.length > 0,
    'Mosque Finder directory has no records',
  );
  const ids = new Set();
  const urls = new Set();
  ...
  assert(
    ...
    'Mosque Finder source count mismatch',
  );
  assert(
    directory.source.sitemapRecordCount >= directory.records.length,
    'Sitemap count cannot be below parsed record count',
  );
  ...
  assert(
    declaredRegions.size === regions.size &&
      [...regions].every((regionCode) => declaredRegions.has(regionCode)),
    'Mosque Finder regional coverage metadata mismatch',
  );
  const missingRegions = new Set(directory.source.missingRegionCodes ?? []);
  for (const regionCode of Object.values(REGION_CODES))
    assert(
      regions.has(regionCode) !== missingRegions.has(regionCode),
      `Mosque Finder missing-region metadata mismatch for ${regionCode}`,
    );
  return directory;
}
...
function sitemapUrls(xml) {
  return [...xml.matchAll(/<loc>([\s\S]*?)<\/loc>/giu)]
    ...
    .filter((url) =>
      /^https:\/\/(?:www\.)?mosque-finder\.com\.au\/mosque\/[^/?#]+(?:\/index\.html|\/?)$/iu.test(
        url,
      ),
    )
    .map((url) => {
      const normalized = url
        .replace('https://www.mosque-finder.com.au', BASE_URL)
        .replace(/\/+$/u, '');
      return normalized.endsWith('/index.html') ? normalized : `${normalized}/index.html`;
    });
}
...
async function fetchDirectory() {
  ...
  for (let index = 0; index < urls.length; index += 1) {
    ...
    try {
      ...
      if (record === null)
        failures.push({ url, reason: 'unparseable or outside Australian bounds' });
    ...
    if ((index + 1) % 25 === 0 || index + 1 === urls.length) {
      console.log(
        `Australian Mosque Finder: ${String(index + 1)}/${String(urls.length)} pages processed.`,
      );
    }
  ...
  const directory = {
    ...
    source: {
      ...
      missingRegionCodes: Object.values(REGION_CODES).filter(
        (regionCode) => !sorted.some((record) => record.address.regionCode === regionCode),
      ),
    },
    records: sorted,
  };
...
function selfTest() {
  ...
  assert(
    !JSON.stringify(record).includes('PrayTimes'),
    'Calculated PrayTimes implementation leaked into stored facts',
  );
  assert(
    !JSON.stringify(record).includes('818-832-5258'),
    'Commented template contact leaked into stored facts',
  );
...
async function main() {
  ...
  if (args.has('--fetch')) {
    ...
    const withPrayerTimes = directory.records.filter(
      (record) => record.prayerTimes !== null,
    ).length;
    const withContact = directory.records.filter(
      (record) => Object.keys(record.contact).length > 0,
    ).length;
    console.log(
  ...
  validateDirectory(directory);
  console.log(
    `Australian Mosque Finder snapshot check passed: ${String(directory.records.length)} records.`,
  );
}
