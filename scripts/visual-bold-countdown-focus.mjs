...
const scenarios = [
  {
    ...
    expectedFocusState: 'pre-iqamah',
    mosqueName: 'Masjid Al Noor Sydney',
  },
  {
    name: 'bold-1080-ar-iqamah-now-long-name',
    width: 1920,
    height: 1080,
    locale: 'ar',
    appTheme: 'dark',
    displayTheme: 'emerald',
    now: Date.parse('2026-08-21T03:30:20.000Z'),
    expectedFocusState: 'iqamah-now',
    mosqueName: 'مسجد ومركز النور الإسلامي والمجتمعي في سيدني لخدمة العائلات والطلاب',
  },
  {
    name: 'bold-4k-en-next-long-name',
    width: 3840,
    height: 2160,
    locale: 'en',
    appTheme: 'dark',
...
async function validateScenario(browser, scenario) {
  ...
  try {
    ...
    await page.goto(`${baseUrl}/?mode=smart-display&template=bold-countdown-focus`, {
      waitUntil: 'networkidle',
    });

    const root = page.locator('.smart-display');
    const board = page.locator('[data-prayer-board-template="bold-countdown-focus"]');
    ...
    if (
      ...
      state.displayTemplate !== 'bold-countdown-focus'
    ) {
    ...
    const focusState = await board.getAttribute('data-focus-state');
    if (focusState !== scenario.expectedFocusState) {
      throw new Error(
        `${scenario.name} expected ${scenario.expectedFocusState}, got ${String(focusState)}`,
      );
    }

    const prayerRows = await page.locator('.bold-countdown-prayer').count();
    if (prayerRows !== 5) {
    ...
    const otherBoards = await page
      .locator(
        '[data-prayer-board-template="heritage-classic"], [data-prayer-board-template="minimal-modern"]',
      )
      .count();
    if (otherBoards !== 0) {
    ...
    if (countdownBox === null || countdownBox.height < minimumCountdownHeight) {
      throw new Error(
        `${scenario.name} countdown is not long-distance readable: ${JSON.stringify(countdownBox)}`,
      );
    }
    ...
    return { name: scenario.name, status: 'passed', boardGeometry, countdownBox, overflow };
  } finally {
    await context.close();
  }
}
...
await writeFile(
  path.join(artifactDirectory, 'bold-countdown-focus-results.json'),
  `${JSON.stringify({ scenarios: results }, null, 2)}\n`,
  'utf8',
);

if (failures.length > 0) {
  throw new Error(`Bold Countdown Focus visual failures: ${JSON.stringify(failures)}`);
}
