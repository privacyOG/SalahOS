...
const scenarios = [
  ...
  {
    name: 'heritage-1080-ar-emerald-long-name',
    width: 1920,
    height: 1080,
    locale: 'ar',
    appTheme: 'dark',
  ...
  {
    name: 'heritage-4k-en-midnight-long-name',
    width: 3840,
    height: 2160,
    locale: 'en',
    appTheme: 'dark',
...
async function validateScenario(browser, scenario) {
  ...
  try {
    ...
    const board = page.locator('[data-prayer-board-template="heritage-classic"]');
    ...
    if (state.displayTheme !== scenario.displayTheme) {
      throw new Error(`${scenario.name} display theme mismatch: ${JSON.stringify(state)}`);
    }
    ...
    const prayerRows = await page.locator('.heritage-classic-prayer-row').count();
    if (prayerRows !== 5) {
...
await writeFile(
  path.join(artifactDirectory, 'heritage-classic-results.json'),
  `${JSON.stringify({ generatedAt: new Date(fixedNow).toISOString(), scenarios: results }, null, 2)}\n`,
  'utf8',
);

if (failures.length > 0) {
  throw new Error(`Heritage Classic visual failures: ${JSON.stringify(failures)}`);
}

console.log(`Heritage Classic visual acceptance passed ${String(results.length)} scenarios.`);
