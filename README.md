...
- **iOS / iPadOS:** Capacitor shell, foreground native location, native persistence, bounded local prayer notifications, private foreground local-Adhan playback and hosted Xcode Simulator compilation are implemented. The current acceptance workflow additionally exercises fresh iPhone/iPad Simulator installation, application-container resolution, launch and explicit terminate/relaunch with screenshot artifacts; physical-device notification/audio/distribution acceptance and network-isolated offline cold start remain separate gates.
...
SalahOS v1 is not tagged yet. Current automated evidence covers the shared quality gate, Android build/emulator path, the permanent 14-scenario browser visual matrix and hosted macOS/Xcode Simulator compilation. The current iOS workflow is being hardened into fresh iPhone/iPad Simulator build/install/launch/relaunch acceptance. Release tagging remains blocked by the exact-revision final regression gate and the explicitly documented target-specific acceptance items that cannot be inferred from emulator, Simulator or browser execution, including physical notification/audio/display behavior.
