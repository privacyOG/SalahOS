...
`src/data/quran-mutashabih-policy-triggers.json` now maintains **126 translation-risk verses**. `src/data/quran-mutashabih-full-audit.json` and `src/data/quran-mutashabih-review-register.json` must contain the same set. **Every one of the 126 trigger verses has an explicit SalahOS 2026 English wording**; no trigger silently falls back to Pickthall.
...
The 126-verse registry is a **translation-risk screen affecting tanzīh**, not a declaration that no other Qur’anic passage can be mutashabih in another sense.
...
The owner reopened the Muhkam/Mutashabih review on 2026-09-21. All 126 trigger rows remain `pending-scholar-review` with no fabricated reviewer. Independent qualified whole-corpus scholarly sign-off therefore remains pending.
