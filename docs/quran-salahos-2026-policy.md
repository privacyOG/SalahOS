...
Automated tooling may verify corpus completeness, trigger/override/register parity, prohibited literal-risk regressions, SalahOS 2026 identity, Uthmani/Hafs/Medina provenance and rendering safeguards. It may not manufacture scholarly approval. Release refs are fail-closed by default until `qualifiedScholarReviewStatus` is explicitly approved. An exact-version project-owner authorization may permit a **prerelease only** while that status is pending, provided the authorization remains machine-readable, version-scoped, requires prerelease publication and leaves all scholarly-review rows and status fields unchanged. Such a prerelease must disclose the pending review and must not be represented as scholarly certification.
