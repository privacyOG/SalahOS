...
This registry is a translation-risk audit, not a claim that these 143 verses exhaust every theological category of mutashabih. The supplied guide separately notes matters whose exact realities or times are known only to Allah.
...
Software validation still verifies all 114 surahs / 6,236 ayat, content identity, Arabic provenance and the 143-trigger parity contract. The prior project-owner whole-corpus attestation is retained as release/editorial provenance, not independent scholarly approval.

Release refs remain fail-closed by default until a named qualified scholar has approved the required whole-corpus scholarly review. A narrowly scoped exception may be recorded for an exact version when the project owner explicitly authorizes a **prerelease** while review is pending. Such an authorization must be machine-readable, version-matched, require prerelease publication, preserve every pending review row and clearly state that it is not scholarly approval. Stable publication continues to require qualified scholarly approval. Physical/software integrity checks and project-owner attestation do not become scholarly approval through this exception.
