...
const copy: Readonly<Record<Locale, ThemeStudioCopy>> = {
  en: {
    ...
    partialWarning: 'Bulk updates are independent per display; any failed revision remains visible after refresh.',
    ...
    revokeConfirm: 'Revoke this display? It will stop receiving managed updates until enrolled again.',
  ...
  ar: {
    ...
    historyHelp: 'يحفظ جهاز الإدارة آخر الإصدارات المرصودة. تحميل إصدار أقدم ينشئ مسودة استرجاع، والنشر يرفع رقم الإصدار دائماً.',
  ...
  tr: {
    ...
    subtitle: 'Cami ekran temalarını tek bir yönetim alanından tasarlayın, önizleyin, atayın ve yayınlayın.',
    ...
    backgroundHelp: 'Yönetilen yayın yalnızca birinci taraf yerleşik görselleri kabul eder; yerel resimler sessizce yüklenmez.',
    ...
    partialWarning: 'Toplu güncellemeler ekran başına bağımsızdır; başarısız revizyonlar yenilemeden sonra görünür.',
    ...
    historyHelp: 'Bu yönetim cihazı son görülen revizyonları tutar. Eski bir revizyon geri alma taslağı olur; yayın her zaman yeni bir revizyona ilerler.',
  ...
  id: {
    ...
    subtitle: 'Rancang, pratinjau, tetapkan, dan publikasikan tema layar masjid dari satu ruang administrasi.',
    ...
    backgroundHelp: 'Publikasi terkelola hanya menerima karya bawaan pihak pertama; gambar lokal tidak diunggah diam-diam.',
    ...
    partialWarning: 'Pembaruan massal berjalan per layar; revisi yang gagal tetap terlihat setelah penyegaran.',
    ...
    historyHelp: 'Perangkat admin ini menyimpan revisi terakhir yang diamati. Memuat revisi lama membuat draf rollback; publikasi selalu maju ke revisi baru.',
...
const moduleLabels: Readonly<Record<Locale, Readonly<Record<PrayerBoardModuleId, string>>>> = {
  en: {
    'current-time': 'Current time', dates: 'Dates', 'next-prayer': 'Next prayer', countdown: 'Countdown',
    'prayer-timetable': 'Five-prayer timetable', jumuah: "Jumu'ah", 'sunrise-sunset': 'Sunrise / sunset',
    'mosque-branding': 'Mosque branding', announcements: 'Announcements', weather: 'Weather',
  },
  ar: {
    'current-time': 'الوقت الحالي', dates: 'التواريخ', 'next-prayer': 'الصلاة التالية', countdown: 'العد التنازلي',
    'prayer-timetable': 'جدول الصلوات الخمس', jumuah: 'الجمعة', 'sunrise-sunset': 'الشروق / الغروب',
    'mosque-branding': 'هوية المسجد', announcements: 'الإعلانات', weather: 'الطقس',
  },
  tr: {
    'current-time': 'Mevcut saat', dates: 'Tarihler', 'next-prayer': 'Sonraki namaz', countdown: 'Geri sayım',
    'prayer-timetable': 'Beş vakit tablosu', jumuah: 'Cuma', 'sunrise-sunset': 'Gün doğumu / batımı',
    'mosque-branding': 'Cami markalaması', announcements: 'Duyurular', weather: 'Hava durumu',
  },
  id: {
    'current-time': 'Waktu saat ini', dates: 'Tanggal', 'next-prayer': 'Salat berikutnya', countdown: 'Hitung mundur',
    'prayer-timetable': 'Jadwal lima salat', jumuah: 'Jumat', 'sunrise-sunset': 'Terbit / terbenam',
    'mosque-branding': 'Identitas masjid', announcements: 'Pengumuman', weather: 'Cuaca',
  },
};
...
function targetFingerprint(display: ManagedDisplayRemoteStatus, config: PrayerBoardTemplateConfig): string {
...
function TemplateThumbnail({ templateId }: Readonly<{ templateId: PrayerBoardTemplateId }>) {
  return (
    <span className={`prayer-board-template-thumbnail prayer-board-template-thumbnail--${templateId}`} aria-hidden="true">
      ...
      <span className="prayer-board-template-thumbnail__rows"><i /><i /><i /><i /><i /></span>
    </span>
  );
}

function LivePreview({ config, label }: Readonly<{ config: PrayerBoardTemplateConfig; label: string }>) {
...
export function AdminDisplayThemeManagement() {
  ...
  const [enrollment, setEnrollment] = useState({
    displayId: '', organizationId: '', mosqueId: '', locationId: '', orientation: 'landscape' as const,
    resolutionProfile: '1920x1080', playlistId: '',
  });
  ...
  const compatibleDisplays = useMemo(
    () => displaysForMosque.filter((display) => resolveManagedPrayerBoardTarget(display.identity).supported && !display.remoteConfig.revoked),
    [displaysForMosque],
  );
  ...
  const mosquePreviewReady =
    requiredMosquePreviewKeys.length > 0 && requiredMosquePreviewKeys.every((key) => previewedTargets.has(key));
  const selectedPreviewReady =
    selectedPlan.supported.length > 0 &&
    selectedPlan.unsupported.length === 0 &&
    [...new Set(selectedPlan.supported.map((display) => targetFingerprint(display, draft)))].every((key) => previewedTargets.has(key));
  ...
  const rememberFleet = useCallback((nextDisplays: readonly ManagedDisplayRemoteStatus[], nextDefaults: readonly ManagedMosquePrayerBoardDefault[]) => {
    for (const entry of nextDefaults) {
      recordManagedThemeRevision(storage, {
        scope: 'mosque-default', targetId: entry.mosqueId, revision: entry.revision,
        prayerBoardConfig: entry.prayerBoardConfig, observedAt: entry.updatedAt,
      });
    }
    for (const display of nextDisplays) {
      if (display.remoteConfig.prayerBoardAssignment !== 'display-override') continue;
      recordManagedThemeRevision(storage, {
        scope: 'display-override', targetId: display.identity.displayId,
  ...
  const refresh = useCallback(async (activeClient: ManagedAdminClient) => {
    const [nextDisplays, nextDefaults] = await Promise.all([
      activeClient.listDisplays(), activeClient.listMosquePrayerBoardDefaults(),
    ]);
    setDisplays(nextDisplays);
    setMosqueDefaults(nextDefaults);
    rememberFleet(nextDisplays, nextDefaults);
    const nextMosque = selectedMosqueId !== '' && nextDisplays.some((display) => display.identity.mosqueId === selectedMosqueId)
      ? selectedMosqueId
  ...
  const previewTarget = (display: ManagedDisplayRemoteStatus) => {
    ...
    setTargetPreview({ identity: display.identity, config: createManagedPrayerBoardAssignmentConfig(draft) });
  };
  ...
  const publishMosqueDefault = async () => {
    ...
    try {
      if (mosqueDefault !== null) {
        recordManagedThemeRevision(storage, {
          scope: 'mosque-default', targetId: mosqueDefault.mosqueId, revision: mosqueDefault.revision,
          prayerBoardConfig: mosqueDefault.prayerBoardConfig, observedAt: mosqueDefault.updatedAt,
        });
      }
      ...
      recordManagedThemeRevision(storage, {
        scope: 'mosque-default', targetId: published.mosqueId, revision: published.revision,
        prayerBoardConfig: published.prayerBoardConfig, observedAt: published.updatedAt,
      });
      setHistoryEpoch((value) => value + 1);
  ...
  const publishBulkOverride = async () => {
    ...
    try {
      ...
      if (failed > 0) setError(`${String(failed)} / ${String(results.length)} updates failed. ${text.partialWarning}`);
      else {
        setStatus(text.published);
        setRollbackSource(null);
      }
  ...
  const inheritBulk = async () => {
    if (client === null || selectedPlan.supported.length === 0 || selectedPlan.unsupported.length > 0) return;
    setBusy(true);
    try {
      const results = await Promise.allSettled(
  ...
  const loadRollback = (scope: ManagedThemeRevisionScope, targetId: string, revision: number) => {
    const snapshot = listManagedThemeRevisions(storage, scope, targetId).find((entry) => entry.revision === revision);
  ...
  const revokeDisplay = async (display: ManagedDisplayRemoteStatus) => {
    if (client === null || display.remoteConfig.revoked || !window.confirm(text.revokeConfirm)) return;
    setBusy(true);
    try {
  ...
  const setModule = (moduleId: PrayerBoardModuleId, visible: boolean) => {
    ...
    replaceDraft({ ...draft, moduleVisibility: { ...draft.moduleVisibility, [moduleId]: visible } });
  };

  return (
    <section className="admin-display-theme-studio" aria-labelledby="admin-display-theme-title" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <section className="admin-theme-connection" aria-labelledby="admin-theme-connection-title">
        ...
        <div className="admin-theme-connection__fields">
          <label><span>{text.endpoint}</span><input type="url" value={baseUrl} autoComplete="off" spellCheck={false} onChange={(event) => setBaseUrl(event.target.value)} /></label>
          <label><span>{text.token}</span><input type="password" value={adminToken} autoComplete="off" spellCheck={false} onChange={(event) => setAdminToken(event.target.value)} /></label>
        </div>
        <div className="admin-theme-actions">
          <button type="button" disabled={busy} onClick={() => void connect()}>{text.connect}</button>
          <button type="button" disabled={client === null || busy} onClick={() => {
            setManagedAdminSession(null); setClient(null); setDisplays([]); setMosqueDefaults([]); setSelectedDisplayIds(new Set()); setStatus(text.disconnected);
          }}>{text.disconnect}</button>
        </div>
      </section>

      {(status !== '' || error !== '') && (
        <div className="admin-theme-feedback" role="status" data-error={error !== '' ? 'true' : 'false'}>
      ...
      <section className="admin-theme-gallery-section" aria-labelledby="admin-theme-gallery-title">
        <div className="admin-theme-section-heading">
          ...
          {rollbackSource !== null && <span>↶ {text.revision} {rollbackSource}</span>}
        </div>
        <div className="prayer-board-template-gallery">
          {prayerBoardTemplateRegistry.map((template) => (
            <button key={template.id} type="button" className="prayer-board-template-card" data-template-card={template.id} aria-pressed={draft.templateId === template.id} onClick={() => replaceDraft(changeManagedThemeTemplate(draft, template.id))}>
              <TemplateThumbnail templateId={template.id} />
              <span className="prayer-board-template-card__copy"><strong>{template.label}</strong><small>{draft.templateId === template.id ? text.selected : ''}</small></span>
            </button>
          ))}
        </div>
      </section>
      ...
      <div className="admin-theme-workbench">
        <section className="admin-theme-config" aria-labelledby="admin-theme-config-title">
          ...
          <div className="admin-theme-config__grid">
            <label><span>{text.primaryLanguage}</span><select value={draft.primaryLocale} onChange={(event) => replaceDraft({ ...draft, primaryLocale: event.target.value })}><option value="en">English</option><option value="ar">العربية</option><option value="tr">Türkçe</option><option value="id">Indonesia</option></select></label>
            <label><span>{text.clock}</span><select value={draft.timeFormat} onChange={(event) => replaceDraft({ ...draft, timeFormat: event.target.value })}><option value="h23">24-hour</option><option value="h12">12-hour</option></select></label>
            <label><span>{text.accent}</span><select value={draft.accentPreset} onChange={(event) => replaceDraft({ ...draft, accentPreset: event.target.value })}>{ACCENTS.map((accent) => <option value={accent} key={accent}>{accent}</option>)}</select></label>
            <label><span>{text.background}</span><select value={draft.background.kind === 'builtin' ? draft.background.artworkId : getPrayerBoardTemplate(draft.templateId).fallbackArtworkId} onChange={(event) => replaceDraft(changeManagedThemeArtwork(draft, event.target.value as PrayerBoardArtworkId))}>{ARTWORK_OPTIONS.map((artwork) => <option value={artwork.id} key={artwork.id}>{artwork.label}</option>)}</select></label>
          </div>
          <p className="admin-theme-config__help">{text.backgroundHelp}</p>
          <fieldset className="admin-theme-branding"><legend>{text.branding}</legend><div className="admin-theme-branding__grid">{(['en', 'ar', 'tr', 'id'] as const).map((language) => <label key={language}><span>{language.toUpperCase()}</span><input dir={language === 'ar' ? 'rtl' : 'auto'} value={draft.branding.mosqueName?.[language] ?? ''} onChange={(event) => replaceDraft(changeManagedThemeMosqueName(draft, language, event.target.value))} /></label>)}</div></fieldset>
          <fieldset className="admin-theme-modules"><legend>{text.modules}</legend><div>{OPTIONAL_MODULES.map((moduleId) => <label key={moduleId}><input type="checkbox" checked={draft.moduleVisibility[moduleId]} onChange={(event) => setModule(moduleId, event.target.checked)} /><span>{moduleLabels[locale][moduleId]}</span></label>)}</div></fieldset>
        </section>
        <section className="admin-theme-preview-panel" aria-labelledby="admin-theme-live-title"><h3 id="admin-theme-live-title">{text.livePreview}</h3><LivePreview config={draft} label={text.livePreview} /></section>
      </div>

      <section className="admin-theme-fleet" aria-labelledby="admin-theme-fleet-title">
        <div className="admin-theme-section-heading"><div><h3 id="admin-theme-fleet-title">{text.fleet}</h3><p>{text.partialWarning}</p></div><div className="admin-theme-actions"><button type="button" disabled={compatibleDisplays.length === 0} onClick={() => setSelectedDisplayIds(new Set(compatibleDisplays.map((display) => display.identity.displayId)))}>{text.selectCompatible}</button><button type="button" disabled={selectedDisplayIds.size === 0} onClick={() => setSelectedDisplayIds(new Set())}>{text.clearSelection}</button></div></div>
        {mosqueIds.length > 0 && <label className="admin-theme-mosque-picker"><span>{text.mosque}</span><select value={selectedMosqueId} onChange={(event) => { const next = event.target.value; setSelectedMosqueId(next); setSelectedDisplayIds(new Set()); setPreviewedTargets(new Set()); const found = mosqueDefaults.find((entry) => entry.mosqueId === next); if (found !== undefined) setDraft(found.prayerBoardConfig); }}><option value="">—</option>{mosqueIds.map((id) => <option value={id} key={id}>{id}</option>)}</select><small>{mosqueDefault === null ? `${text.revision}: —` : `${text.revision}: ${String(mosqueDefault.revision)}`}</small></label>}

        {displaysForMosque.length === 0 ? <p>{client === null ? text.disconnected : text.noCompatible}</p> : <div className="admin-theme-display-grid">{displaysForMosque.map((display) => { const target = resolveManagedPrayerBoardTarget(display.identity); const selectable = target.supported && !display.remoteConfig.revoked; const checked = selectedDisplayIds.has(display.identity.displayId); return <article className="admin-theme-display-card" key={display.identity.displayId} data-compatible={target.supported ? 'true' : 'false'}><header><label><input type="checkbox" disabled={!selectable} checked={checked} onChange={(event) => setSelectedDisplayIds((current) => { const next = new Set(current); if (event.target.checked) next.add(display.identity.displayId); else next.delete(display.identity.displayId); return next; })} /><strong dir="ltr">{display.identity.displayId}</strong></label><span>{target.supported ? text.compatible : text.blocked}</span></header><dl><div><dt>{text.target}</dt><dd dir="ltr">{target.width > 0 ? `${target.width}×${target.height}` : display.identity.resolutionProfile} · {display.identity.orientation}</dd></div><div><dt>{text.currentDesign}</dt><dd>{getPrayerBoardTemplate(display.remoteConfig.prayerBoardConfig.templateId).label}</dd></div><div><dt>{text.source}</dt><dd>{display.remoteConfig.prayerBoardAssignment}</dd></div><div><dt>{text.revision}</dt><dd>{display.remoteConfig.contentRevision}</dd></div></dl>{!target.supported && <p>{target.reason}</p>}<div className="admin-theme-display-card__actions"><button type="button" disabled={!selectable} onClick={() => previewTarget(display)}>{text.exactPreview}</button><button className="ds-button ds-button--destructive" type="button" disabled={display.remoteConfig.revoked || busy} onClick={() => void revokeDisplay(display)}>{text.revoke}</button></div></article>; })}</div>}

        <div className="admin-theme-publication-bar"><span>{text.selectedDisplays}: <strong>{selectedDisplayIds.size}</strong></span><button type="button" disabled={client === null || busy || !mosquePreviewReady} onClick={() => void publishMosqueDefault()}>{text.publishMosque}</button><button type="button" disabled={client === null || busy || !selectedPreviewReady} onClick={() => void publishBulkOverride()}>{text.publishBulk}</button><button type="button" disabled={client === null || busy || selectedPlan.supported.length === 0 || selectedPlan.unsupported.length > 0} onClick={() => void inheritBulk()}>{text.inheritBulk}</button></div>
        {compatibleDisplays.length > 0 && !mosquePreviewReady && <p className="admin-theme-preview-requirement">{text.previewRequired}</p>}
      </section>

      <section className="admin-theme-history" aria-labelledby="admin-theme-history-title">
        <div className="admin-theme-section-heading"><div><h3 id="admin-theme-history-title">{text.history}</h3><p>{text.historyHelp}</p></div><label><span>{text.target}</span><select value={historyTarget === null ? '' : `${historyTarget.scope}|${historyTarget.targetId}`} onChange={(event) => { const [scope, ...rest] = event.target.value.split('|'); const targetId = rest.join('|'); setHistoryTarget(scope === 'mosque-default' || scope === 'display-override' ? { scope, targetId } : null); }}><option value="">—</option>{mosqueDefaults.map((entry) => <option key={`mosque-${entry.mosqueId}`} value={`mosque-default|${entry.mosqueId}`}>{text.mosque}: {entry.mosqueId}</option>)}{displays.filter((display) => display.remoteConfig.prayerBoardAssignment === 'display-override').map((display) => <option key={`display-${display.identity.displayId}`} value={`display-override|${display.identity.displayId}`}>{display.identity.displayId}</option>)}</select></label></div>
        {historyTarget === null || history.length === 0 ? <p>{text.noHistory}</p> : <div className="admin-theme-history__list">{history.map((entry, index) => <article key={`${entry.scope}-${entry.targetId}-${entry.revision}`}><div><strong>{text.revision} {entry.revision}</strong><span>{getPrayerBoardTemplate(entry.prayerBoardConfig.templateId).label} · {new Intl.DateTimeFormat(undefined, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(entry.observedAt))}</span></div>{index === 0 ? <span>{text.current}</span> : <button type="button" onClick={() => loadRollback(entry.scope, entry.targetId, entry.revision)}>{text.loadRevision}</button>}</article>)}</div>}
      </section>

      <details className="admin-theme-enrollment"><summary>{text.enroll}</summary><div className="admin-theme-enrollment__grid"><label><span>{text.displayId}</span><input value={enrollment.displayId} onChange={(event) => setEnrollment({ ...enrollment, displayId: event.target.value })} /></label><label><span>{text.organizationId}</span><input value={enrollment.organizationId} onChange={(event) => setEnrollment({ ...enrollment, organizationId: event.target.value })} /></label><label><span>{text.mosqueId}</span><input value={enrollment.mosqueId} onChange={(event) => setEnrollment({ ...enrollment, mosqueId: event.target.value })} /></label><label><span>{text.locationId}</span><input value={enrollment.locationId} onChange={(event) => setEnrollment({ ...enrollment, locationId: event.target.value })} /></label><label><span>{text.orientation}</span><select value={enrollment.orientation} onChange={(event) => setEnrollment({ ...enrollment, orientation: event.target.value as 'landscape' })}><option value="landscape">landscape</option><option value="portrait">portrait</option></select></label><label><span>{text.profile}</span><input value={enrollment.resolutionProfile} onChange={(event) => setEnrollment({ ...enrollment, resolutionProfile: event.target.value })} /></label><label><span>{text.playlist}</span><input value={enrollment.playlistId} onChange={(event) => setEnrollment({ ...enrollment, playlistId: event.target.value })} /></label></div><button type="button" disabled={client === null || busy} onClick={() => void enrollDisplay()}>{text.enrollAction}</button>{credential !== null && <aside className="admin-theme-credential" role="status"><strong>{text.credential}</strong><code dir="ltr">{credential}</code><span>{text.credentialWarning}</span></aside>}</details>

      {targetPreview !== null && <ManagedDisplayTargetPreview identity={targetPreview.identity} config={targetPreview.config} locale={locale} onClose={() => setTargetPreview(null)} />}
    </section>
  );
}
