...
describe('local Adhan audio', () => {
  ...
  it('returns one stable foreground playback key at an enabled Adhan prayer minute', () => {
  ...
  it('keeps Adhan selection independent from ordinary notification enablement', () => {
    const notifications = updatePrayerNotificationPreference(
      defaultNotificationPreferences,
      'fajr',
    ...
    expect(
      foregroundAdhanPlaybackKey({
        date: '2026-08-16',
        localMinutes: 330,
        prayers: [{ name: 'fajr', localMinutes: 330 }],
        notifications,
      }),
    ).toBe('2026-08-16:fajr:local-adhan');
  });

  it('does not trigger for disabled Adhan or sunrise', () => {
    const prayers = [
      { name: 'fajr', localMinutes: 330 },
      { name: 'sunrise', localMinutes: 410 },
    ] as const;
